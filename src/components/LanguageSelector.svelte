...
>
  ...
  {#each Object.entries(translations) as [lang, slug] (lang)}
    <a
      href={`/posts/${slug}`}
      class="inline-flex items-center gap-1.5 px-2 py-1 rounded text-xs font-bold uppercase tracking-wide bg-background text-foreground hover:text-primary border border-border transition-colors no-underline"
    >
